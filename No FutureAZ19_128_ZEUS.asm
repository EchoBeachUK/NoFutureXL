...
START:                  ; call DISPLAY_COCKPIT          ;
                        ...
                        LD SP,Zeus_SP                   ; Set general stack pointer
                        XOR A                           ; A=0 to set border black
                        LD (23624),A                    ; Set border system varable black
                      ;  CALL 8859                       ; Set border black  (NO GOOD ON 128)


                        CALL MEMORY_SWITCH_0            ; Switch to memory 0

                        LD IX,PRESS_ANY_KEY_0           ; Point to Press any key string to display it on start image
                        CALL DISPLAY_STRING             ; Display string

                        CALL MUSIC_0                    ; Play title music and wait for any key
                        CALL FADE_CLEAR_PLAY_AREA       ; Clear play area
                        CALL RESET_ALL_SOUNDS           ; Reset Sound
                        CALL RESET_STATUS               ; Reset score and lives figures and display with Hi score.

;Display first Cefay screen
DISPLAY_NEWS:
                        ...
                        CALL COLOUR_FULL_PLAY_AREA      ; Colour the play area
                        ...
                        CALL MEMORY_SWITCH_0            ; Switch to memory 0
                        LD IX,START_MESSAGEA_0          ; "p197 SBCFAX 197 Mon 20 Jun"
                        CALL DISPLAY_STRING             ; Display string
...
; Set alien head initial status
                        ...
                        CALL MEMORY_SWITCH_3            ; Switch to memory 3
...
ALIEN_BACKGROUND_COLOURB:LD L,0                         ; Use HL or whichever 16-bit register you prefer
                        ...
                        LD B,19                         ; 5 top lines to fill for ATTR buffer
; Fill the memory
PUSHLOOP2:
                        PUSH HL                         ; Writes HL to (SP-2) and DECs SP by 2
                        PUSH HL                         ; Writes HL to (SP-2) and DECs SP by 2
...
; Check to open portal
                        ...
                        CP 12                           ; Is it at 12?
                        JR Z,SKIP_ADD_TO_PORTAL         ; If so then jump to skip adding more to portal appear counter
...
;Get screen address for pixel
                        LD A,D                          ; Up/down coordinate 175 Test that the y co-ordinate (in SUB B B) is not greater than 116.
                        ...
                        AND 7                           ; A holds x(mod 8): so the pixel is bit (A - 7) within the byte.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
ALIENHEAD_COMING_OUT:
                        ...
                        LD A,201                        ; RET command
                        LD (BUFFER_RETURN),A            ; Set command at BUFFER_RETURN to RET
...
SHOW_ALIEN_IMAGE_LOOP1:
                        ...
                        XOR A                           ; A=0
                        LD (BUFFER_RETURN),A            ; Set BUFFER_RETURN to NOP
...
ALIEN_ENABLED_TEST_LOOP:
                        ...
                        JR NZ,SKIP_CLOSE_PORTAL         ; If not then return as there are still some aliens enabled
...
; Pow bonus
                        ...
                        LD A,(ALIEN_HIT_COUNTER)        ; Get alien hit counter
                        CP 20                            ; Reference level 3
                        RET C                           ; Jump Pow if alien hit counter is below 5
...
; Multi player missile bonus
                        ...
                        LD A,(ALIEN_HIT_COUNTER)        ; Get alien hit counter
                        CP 5                            ; Reference level 3
                        RET C                           ; Jump Rockets if alien hit counter is below 5
...
SKIP_RESETTING_WAVE_TO_0:
                        ...
                        LD (SET_COLOUR+1),A             ; Set the background colour
                        CALL COLOUR_PLAY_AREA           ; Colour background to change text colour
...
; Display Next Level Message
                        ...
                        LD B,10                         ; Scroll 8 lines
SCROLL_NEXT_LEVEL_LOOP:
                        PUSH BC                         ; Save scroll loop

                        LD BC,2000                      ; Set delay time
                        CALL DELAY                      ; Delay
                        CALL SCROLL_BOX                 ; Scroll Next Phase and Game Over up screen
                        LD BC,2000                      ; Set delay time
                        CALL DELAY                      ; Delay
                        CALL SCROLL_BOX                 ; Scroll Next Phase and Game Over up screen
                        LD BC,2000                      ; Set delay time
                        ...
                        POP BC                          ; Restore scroll loop counter
                        DJNZ SCROLL_NEXT_LEVEL_LOOP     ; Jump back to scroll another line
...
GAME_OVER_TEXT:
                        DEFB 12,7,71,1                  ; Line,Collumn,Colour,1=Double Height
...
; Next level message
NEXT_LEVEL_MESSAGE:
                        DEFB 12,4,66,1                  ; Line,Collumn,Colour,1=Double Height
                        ;  DEFM "                  #"      ; String to display
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Scroll Next Phase and Game Over up screen
SCROLL_BOX:
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Reset score and lives figures and display with Hi score.
RESET_STATUS:
                        LD HL,LIVES                     ; 3 lives to start with
                        LD (HL),19                      ; Set lives
                        CALL DISPLAY_LIVES_FIGURES              ; Display lives at top of screen
